...
namespace QuickBooksIntegration.Tests
{
  class OAuthWorkFlow
  {
    ...
    public async Task RefreashAccessTokenAndGetResource()
    {
      var data = new Dictionary<string, string>
      {
        {"code", "Q011508911870v6HbgJEoTFuua9qnYx8RWePaKAFMA7KxTCXbs" },
        {"grant_type", "authorization_code" },
        {"redirect_uri", "https://developer.intuit.com/v2/OAuth2Playground/RedirectUrl" }
      };
      var authClient = new HttpClient() { BaseAddress = new Uri("https://oauth.platform.intuit.com") };
      ...
      Console.WriteLine($"Access token from access code\n{jwtToken}");

      data = new Dictionary<string, string>
      {
        {"grant_type", "refresh_token" },
        {"refresh_token", (string)jwtToken.refresh_token }
      };

      var getAccessTokenWithRefreshResponse = await authClient.PostAsync("oauth2/v1/tokens/bearer", new FormUrlEncodedContent(data));

      var jwtTokenFromRefresh = await getAccessTokenWithRefreshResponse.Content.ReadAsAsync<dynamic>();

      Console.WriteLine($"Access token from refresh token\n{jwtTokenFromRefresh}");

      string accessToken = jwtTokenFromRefresh.access_token;
